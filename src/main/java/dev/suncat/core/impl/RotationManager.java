...
import dev.suncat.api.events.impl.DoAttackEvent;
import dev.suncat.api.events.impl.FireworkShooterRotationEvent;
import dev.suncat.api.events.impl.InteractBlockEvent;
import dev.suncat.api.events.impl.InteractItemEvent;
...
import dev.suncat.mod.modules.impl.movement.HoleSnap;
import dev.suncat.mod.modules.impl.player.Freecam;
import dev.suncat.mod.modules.settings.enums.SnapBack;
...
implements Wrapper {
    ...
    public void update(SendMovementPacketsEvent event) {
        if (AntiCheat.INSTANCE.movementSync() && !BaritoneUtil.isActive()) {
            ...
            suncat.EVENT_BUS.post(updateRotateEvent);
    ...
    public void onLastRotation(UpdateRotateEvent event) {
        ...
        suncat.EVENT_BUS.post(rotationEvent);
    ...
    public void setLastYaw(float lastYaw) {
        ...
        if (AntiCheat.INSTANCE.forceSync.getValue() && suncat.SERVER.playerNull.passedS(0.15)) {
    ...
    // Called in Soft/Strong mode for rotation update
    public void onUpdate() {
        // Can be extended for specific rotation logic in Soft/Strong mode
    }

    // Get current rotation for strafeFix and visualize
