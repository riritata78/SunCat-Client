...
import dev.suncat.api.events.impl.ClickBlockEvent;
...
import dev.suncat.mod.modules.impl.combat.CevBreaker;
...
private final BooleanSetting antiFeetMine = this.add(new BooleanSetting("AntiFeetMine", false, () -> this.page.is(Page.Check)));
    ...
    float getDigSpeed(BlockState state, ItemStack itemStack) {
        ...
        boolean bl = inWeb = this.checkWeb.getValue() && suncat.PLAYER.isInWeb((PlayerEntity)PacketMine.mc.player) && PacketMine.mc.world.getBlockState(this.breakPos).getBlock() == Blocks.COBWEB;
