...
import dev.suncat.asm.accessors.IFireworkRocketEntity;
...
extends Module {
    ...
    public ElytraFly() {
        ...
        INSTANCE = this;
        suncat.EVENT_BUS.subscribe(new FireWorkTweak());
    }
    ...
    public void off() {
        ...
        if (ElytraFly.mc.player.getMainHandStack().getItem() == Items.FIREWORK_ROCKET) {
            if (this.packetInteract.getValue()) {
                ElytraFly.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, id, suncat.ROTATION.getLastYaw(), suncat.ROTATION.getLastPitch()));
            } else {
