...
import dev.suncat.api.utils.math.PredictUtil;
            ...
            for (float x : new float[]{0.0f, this.offset.getValueFloat(), -this.offset.getValueFloat()}) {
                for (float z : new float[]{0.0f, this.offset.getValueFloat(), -this.offset.getValueFloat()}) {
                    for (float y : new float[]{0.0f, 1.0f, -1.0f}) {
                        ...
                        list.add(pos);
                        if (!this.isTargetHere(pos, player) || AutoWeb.mc.world.getBlockState((BlockPos)pos).getBlock() != Blocks.COBWEB || suncat.BREAK.isMining(pos)) continue;
                        ++webs;
                    }
                }
            }
