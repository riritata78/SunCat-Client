...
import dev.suncat.api.utils.entity.PlayerEntityPredict;
import dev.suncat.api.utils.math.AnimateUtil;
import dev.suncat.api.utils.math.Animation;
import dev.suncat.api.utils.math.DamageUtils;
...
extends Module {
    ...
    public AutoCrystal() {
        ...
        INSTANCE = this;
        suncat.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void calcCrystalPos() {
        ...
        if (list.isEmpty()) {
            ...
            for (BlockPos pos : BlockUtil.getSphere((float)this.breakRange.getValue() + 1.5f)) {
                ...
                if (base && suncat.BREAK.isMining(pos.down()) && this.detectMining.getValue() || (attackVec = this.getAttackVec(pos.toBottomCenterPos())) == null || this.behindWall(pos, attackVec) || !this.canTouch(pos.down()) || !this.canPlaceCrystal(pos, true, false)) continue;
    ...
    private void doBreak(EndCrystalEntity entity) {
        ...
        if ((crystalPos = this.crystalPos) != null && this.displayTarget != null && (double)this.lastDamage >= this.getDamage(this.displayTarget) && this.afterBreak.getValue() && (!this.rotate.getValue() || !this.shouldYawStep() || !this.checkFov.getValue() || suncat.ROTATION.inFov(entity.getPos(), this.fov.getValueFloat()))) {
